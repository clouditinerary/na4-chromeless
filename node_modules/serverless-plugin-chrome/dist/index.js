    ...
    return _asyncToGenerator(function* () {
      ...
      if (_this2.messWithServicePath) {
        ...
        if (!fs.existsSync(path.resolve(path.join(BUILD_FOLDER, 'node_modules')))) {
          fs.symlinkSync(path.resolve('node_modules'), path.resolve(path.join(BUILD_FOLDER, 'node_modules')));
        }
        ...
        files.forEach(function (filename) {
          ...
          const dirname$$1 = path.dirname(destFileName);

          if (!fs.existsSync(dirname$$1)) {
            fs.mkdirpSync(dirname$$1);
          }
      ...
      yield Promise.all(functionsToWrap.map((() => {
        var _ref = _asyncToGenerator(function* (functionName) {
          ...
          });const wrapperTemplate = yield utils.readFile(path.resolve(__dirname, '..', 'src', `wrapper-${providerName}-${runtime}.js`));
